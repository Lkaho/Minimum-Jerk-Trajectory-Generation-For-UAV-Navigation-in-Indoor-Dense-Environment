...
ros::Publisher _traj_vis_pub, _traj_pub, _path_vis_pub, _pre_path_vis_pub;
...
void visPrePath(vector<Vector3d>& grid_path);
...
void execCallback(const ros::TimerEvent &e) {
  ...
  case REPLAN_TRAJ: {
    ros::Time time_now = ros::Time::now();
    double t_cur = (time_now - time_traj_start).toSec();
    double t_delta = ros::Duration(0, 30).toSec();
    t_cur = t_delta + t_cur;
...
// back-end  : Minimum jerk trajectory generation
bool trajGeneration() {
  /**
   *  search the path and get the path
   * **/
  _astar_path_finder->AstarGraphSearch(start_pt, target_pt);
  auto grid_path = _astar_path_finder->getPath();
  _astar_path_finder->resetUsedGrids();
  visPrePath(grid_path);

  // Reset map for next call

  /**
   * Simplify the path: use the RDP algorithm
   * **/
 ...
 std::cout << "_______grid_path________" << std::endl;
...
void trajOptimization(Eigen::MatrixXd path) {
  // if( !has_odom ) return;
  Eigen::Vector3d initialVel = start_vel;
  Eigen::Vector3d initialAcc = Eigen::Vector3d::Zero();
  Eigen::Vector3d terminalVel = Eigen::Vector3d::Zero();
  Eigen::Vector3d terminalAcc = Eigen::Vector3d::Zero();


  _polyTime = timeAllocation(path);
  int pieceNum = _polyTime.size();
  Eigen::MatrixXd coefficientMatrix;
  
  _trajGene->minimumJerkTrajGen(pieceNum, initialVel, 
                                                                          initialAcc, terminalVel,
                                                                          terminalAcc, path,
                                                                          _polyTime, coefficientMatrix);
                                                                          
  // check if the trajectory is safe, if not, do reoptimize
  int unsafe_segment;

  //check  collision segment
  unsafe_segment = _astar_path_finder->safeCheck(coefficientMatrix, _polyTime);

  MatrixXd repath = path;
  // int count = 0;
  while (unsafe_segment != -1) {

    Vector3d prePt = repath.row(unsafe_segment);
    Vector3d postPt = repath.row(unsafe_segment + 1);
    Vector3d middlePt = (prePt + postPt) / 2;
    MatrixXd temp(repath.rows() + 1, repath.cols());
    for(int i = 0; i <= int(repath.rows()); ++i) { 
      if(i <= unsafe_segment){
        temp.row(i) = repath.row(i);
        continue;
      }else if(i == unsafe_segment + 1 ) {
        temp.row(i) = middlePt;
        continue;
      }else{
         temp.row(i) = repath.row(i -1);
      }
    }
    repath = temp;
    _polyTime = timeAllocation(repath);
    pieceNum = _polyTime.size();
    _trajGene->minimumJerkTrajGen(pieceNum, initialVel, 
                                                                          initialAcc, terminalVel,
                                                                          terminalAcc, repath,
                                                                          _polyTime, coefficientMatrix);
  ...
  pieceNum =_polyTime.size();
  _polyCoeff = MatrixXd::Zero(pieceNum, 3 * _poly_num1D);
  for (int i = 0; i < pieceNum; ++i) {
    for (int j = 0; j < 3; ++j) {
      _polyCoeff.block(i, j * _poly_num1D, 1, _poly_num1D) = coefficientMatrix.block(6 * i, j, 6, 1).transpose();
    }
  }
  // std::cout << "code is running in 335 line" << std::endl;
...
void visPrePath(vector<Vector3d>& grid_path) {
  ...
  _pre_path_vis_pub.publish(points);
}
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  for(int i = 0; i < int(Path.rows() - 1); ++i){
    ...
    time[i] = timeTrapzVel(dist, max_vel, max_acc); 
    // time[i] -= 0.05;
  }
...
int main(int argc, char **argv) {
  ...
  nh.param("path/resolution", _path_resolution, 0.08);
 ...
 _pre_path_vis_pub = nh.advertise<visualization_msgs::Marker>("vis_pre_path", 1);
  // set the obstacle map
